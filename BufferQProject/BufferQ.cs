...
{
    /// <summary>
    /// Provides methods to take and process items from a threadsafe queue. Queue item should be filled on OnEmpty event let the threads process these items on each seperate thread
    /// </summary>
    /// <typeparam name="T">Type of the object that thread safe queue will remain</typeparam>
    public class BufferQ<T>
    {
        #region Properties
        /// <summary>
        /// Working threads
        ...
        /// <summary>
        /// Provides for calling OnEventEmpty event from one thread in a time
        /// </summary>
        ...
        public int ActiveThreadCount
        {
            get
            {
                return _totalActiveThreadCount;
            }
        }
        #endregion
        ...
        public delegate void OnQueueEmptyHandler(object sender, BufferQEventArgs<T> e);
        /// <summary>
        ...
        {
            Interlocked.Increment(ref _totalActiveThreadCount);
            try
            {
        ...
        {
            Interlocked.Decrement(ref _totalActiveThreadCount);
            try
            {
                if (OnThreadCountChanged != null) OnThreadCountChanged(_totalActiveThreadCount);
            }
        ...
        private void CallEmptyThreadEventOnce()//v1.0.0.1
        {
            ...
            {
                //OnQueueEmpty public event notifies queue is empty
                if (OnQueueEmpty != null) OnQueueEmpty(this, new BufferQEventArgs<T>() { Queue = _queue });
